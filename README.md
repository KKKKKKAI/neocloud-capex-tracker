...
| 4u | Restatement-aware extraction + audit | ✅ | Closes the "latest filing wins" gap. When a company restates a historical period in a later 10-K / 20-F, the new value now supersedes the original across charts, Excel, and audit — and the Excel cell comment cites the *restating* filing. Three capture paths: (a) XBRL `timeseries.py` preserves restated contexts when `accn` differs (new `extracting_model='restated-xbrl'`); (b) new `audit/restatement.py` reads each ticker's latest 10-K via `extract/extractors/restated.py`, parses the segment table, and emits findings where a prior-year column differs from DB by >0.5%; (c) reconcile cascades automatically (derived Q4 = FY − 9M re-runs on restated inputs). Selectors in `interactive_chart._load_annual/_load_quarterly`, `charts.py`, `audit/orchestrator.load_cells`, `reconcile._group_rows` all order by `source_documents.filing_date DESC` so newest filing wins. `capex audit` shows a new "Restatements" section; `capex audit --apply` writes back restated rows; `capex extract --restated --ticker T` is a manual refresh. `coverage.yaml` carries `restatement_policy` blocks per known-restater (MSFT, ORCL, BABA, BIDU, GDS), surfaced in the treatments viewer. Full spec + implementation map: [docs/RESTATEMENT_POLICY.md](docs/RESTATEMENT_POLICY.md). |
