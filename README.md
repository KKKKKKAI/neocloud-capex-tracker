...
| 4k | BIDU quarterly cloud proxy | ✅ | `scripts/extract_bidu_cloud_6k.py` extracts "non-online marketing revenue" from each 6-K as a PROXY for AI Cloud (BIDU does not disclose quarterly AI Cloud standalone). Proxy includes AI Cloud + Apollo + smart-device hardware, with AI Cloud being the dominant component. 18/30 filings extracted (pre-2019 has no usable breakout). Rows carry `extracting_model='bidu-cloud-6k-proxy@0.1.0'` and a proxy caveat in the quote. |
